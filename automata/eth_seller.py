"""
automata/eth_seller.py — ETH 5m dual-side sell-side bot for Polymarket.

Strategy: Pre-Candle Pair-Sum Maker → Adjust → Hail Mary
---------------------------------------------------------
Find NEXT un-started ETH 5m candle, split $10 USDC → 10 Up + 10 Down,
...
Usage:
    python -m automata.eth_seller                       # dry-run
    python -m automata.eth_seller --bet                 # live, $10 budget
    python -m automata.eth_seller --bet --budget 20
    python -m automata.eth_seller --bet --split-mode onchain
    python -m automata.eth_seller --min-pair-sum 1.02   # tighter (2¢) floor
    python -m automata.eth_seller --adjust-grace 10     # tighter (10s) wind-down
"""
...
log = logging.getLogger("automata.eth_seller")

LOG_PATH = Path("experiment") / "logs" / "eth_seller.log"
...
def _fetch_eth_candle() -> Candle | None:
    """
    Find an ETH 5m candle from Gamma API.
    ...
    for delta in (0, 1, 2, 3, -1):
        slug = f"eth-updown-5m-{bucket + delta * 300}"
        try:
            data = requests.get(
...
async def _strategy_loop(
    ...
    while True:
        ...
        if need_refresh:
            new_candle = await asyncio.to_thread(_fetch_eth_candle)
            _state.last_market_refresh = now

            if new_candle is None:
                log.warning("No active ETH 5m candle found — retrying…")
...
async def _poly_ws() -> None:
    """Stream the Polymarket CLOB order book for the active ETH candle tokens."""
    while True:
...
def _render(budget: float, bet: bool) -> str:
    ...
    lines = ["", f"  {BOLD}[{ts}]  ETH 5m Sell Bot — Pre-Candle Pair Maker  [{mode}{BOLD}]{RESET}", SEP]
    ...
    if candle:
        ...
        lines.append(f"  {DIM}Waiting for active ETH 5m candle…{RESET}")
...
async def run(
    ...
    log.info("ETH Seller starting  budget=$%.2f  mode=%s",
             budget, "LIVE" if bet else "DRY-RUN")

    # Pre-warm: discover first candle so WS has tokens immediately
    first_candle = await asyncio.to_thread(_fetch_eth_candle)
    if first_candle is None:
        log.error("No active ETH 5m candle — cannot start")
        sys.exit(1)
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(
        description="ETH 5m sell-side bot — pre-candle pair-sum maker → adjust → hail mary"
    )
