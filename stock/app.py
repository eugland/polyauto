...
def create_app(db_path: str) -> Flask:
    ...
    def _btc_shadow_ro_connect():
        # DuckDB allows N readers OR 1 writer across processes — never both.
        # A direct read-only open here would race with the daemon's RW open
        # (the daemon would crash mid-cycle). Always read from a copied
        # snapshot instead. Stale by at most one daemon poll cycle.
    ...
    # ── BTC Live (real --bet trades) ───────────────────────────────────────────
    _BTC_LOG_FILE = Path(__file__).resolve().parent.parent / "logs" / "btc_pre_candle.log"

    @app.route("/btc-log")
    def btc_log():
        return render_template("btc_log.html")

    @app.route("/api/btc-log")
    def api_btc_log():
        try:
            lines = int(request.args.get("lines", "200"))
        except ValueError:
            lines = 200
        path_str = str(_BTC_LOG_FILE)
        try:
            if not _BTC_LOG_FILE.exists():
                return jsonify({"lines": [], "exists": False, "path": path_str})
            with open(_BTC_LOG_FILE, "r", encoding="utf-8", errors="replace") as f:
                all_lines = f.readlines()
            return jsonify({
                "lines": [_redact(l.rstrip()) for l in all_lines[-lines:]],
                "exists": True,
    ...
    @app.route("/btc-live")
    def btc_live():
        return render_template("btc_live.html")

    @app.route("/api/btc-live/summary")
    def api_btc_live_summary():
        """Stats over `live_trades`. Optional ?strategy=NAME filter; always
        returns a `by_strategy` rollup over every strategy in the table."""
        try:
            from automata import btc_pre_candle as bpc
            strategy = request.args.get("strategy") or None
            where_strat = "AND strategy = ?" if strategy else ""
            ...
            empty = {
                "filter_strategy": strategy,
                "live_strategies_allowed": sorted(bpc.LIVE_STRATEGIES),
                "n_total": 0, "n_settled": 0, "wins": 0, "hit_rate": None,
                "total_pnl_usdc": 0.0, "total_cost_usdc": 0.0,
                "avg_entry": None, "total_shares": 0.0,
                "monthly": [], "by_strategy": [],
                "bet_shares": bpc.BET_SHARES,
                "max_entry_ask": bpc.MAX_ENTRY_ASK,
                "daily_spend_cap": bpc.DAILY_SPEND_CAP,
            }
            if con is None:
                return jsonify(empty)
            try:
                # The table may not exist on a fresh DB that's never seen --bet.
                ...
                head = con.execute(f"""
                    ...
                    WHERE 1=1 {where_strat}
                """, params_strat).fetchone()
                n_total, n_settled, wins, total_pnl, total_cost, total_shares, avg_entry = head
                hit_rate = (float(wins) / float(n_settled)) if n_settled else None
            ...
            return jsonify({
                ...
                "wins":      int(wins or 0),
                "hit_rate":  hit_rate,
                "total_pnl_usdc":  float(total_pnl or 0.0),
                "total_cost_usdc": float(total_cost or 0.0),
                "total_shares":    float(total_shares or 0.0),
                "avg_entry": float(avg_entry) if avg_entry is not None else None,
                "monthly": [
                    {
                        "month": (m.isoformat() if hasattr(m, "isoformat") else str(m))[:7],
                ...
                "by_strategy": [
                    ...
                    for (s, nt, ns, w, p, c, ae) in rollup
                ],
                "bet_shares":      bpc.BET_SHARES,
                "max_entry_ask":   bpc.MAX_ENTRY_ASK,
                "daily_spend_cap": bpc.DAILY_SPEND_CAP,
    ...
    @app.route("/api/btc-live/trades")
    def api_btc_live_trades():
        try:
            try:
                limit = int(request.args.get("limit", "200"))
            except ValueError:
            ...
            try:
                ...
                if "live_trades" not in tables:
                    return jsonify({"trades": []})
                rows = con.execute(f"""
                    SELECT lt.id, lt.placed_at, lt.target_hour_utc, lt.strategy, lt.side,
                           lt.slug, lt.shares, lt.entry_price, lt.cost_usdc,
                           lt.order_id, lt.fire_id,
                           lt.resolved_at, lt.outcome, lt.pnl_usdc,
                           f.candle_open, f.candle_close, f.winner
                    FROM live_trades lt
                    LEFT JOIN fires f ON f.id = lt.fire_id
                    {where_strat}
                    ORDER BY lt.placed_at DESC
                    LIMIT ?
                """, params).fetchall()
                cols = [
                    "id","placed_at","target_hour_utc","strategy","side",
                    "slug","shares","entry_price","cost_usdc",
                    "order_id","fire_id",
                    "resolved_at","outcome","pnl_usdc",
                    "candle_open","candle_close","winner",
                ]
            finally:
                con.close()
            out = []
            for r in rows:
                d = dict(zip(cols, r))
                for k in ("placed_at", "target_hour_utc", "resolved_at"):
                    if d.get(k) is not None and hasattr(d[k], "isoformat"):
                        d[k] = d[k].isoformat()
                out.append(d)
    ...
    @app.route("/api/btc-live/equity")
    def api_btc_live_equity():
        """Per-strategy cumulative USDC PnL series (resolved trades only)."""
        try:
            con = _btc_shadow_ro_connect()
            if con is None:
            ...
            try:
                ...
                if "live_trades" not in tables:
                    return jsonify({"series": []})
                rows = con.execute("""
                    SELECT strategy, resolved_at, pnl_usdc
                    FROM live_trades
