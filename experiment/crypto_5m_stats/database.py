...
# ── Highest-temp market queries ───────────────────────────────────────────────

def query_temp_market_cities(temp_db_path: str) -> dict:
    """Return list of available (city, event_date) pairs from DuckDB."""
    if not os.path.exists(temp_db_path):
        return {"error": "temp_market.db not found — start temp_market_collector first.", "sessions": []}
    try:
        import duckdb
...
def query_temp_market_session(temp_db_path: str, city: str, event_date: str) -> dict:
    """Return full session data: buckets, book snapshots, weather obs, forecasts."""
    if not os.path.exists(temp_db_path):
        return {"error": "temp_market.db not found — start temp_market_collector first."}
    try:
        import duckdb
        con = duckdb.connect(temp_db_path, read_only=True)
        ...
        buckets = [
            {
                ...
                "bucket_lo": r[2], "bucket_hi": r[3],
                "unit": r[4], "direction": r[5],
                "yes_token_id": r[6], "icao": r[7],
                "lat": r[8], "lon": r[9], "tz": r[10],
            }
            for r in buckets_raw
        ]

        # --- book snapshots (all buckets, time-ordered) ---
        ...
        # --- forecasts ---
        fc_raw = con.execute("""
            SELECT sampled_at, forecast_high_c, source
            FROM forecasts
        ...
        # --- latest book state per bucket (for table) ---
        latest_raw = con.execute("""
            SELECT bs.condition_id, bs.best_bid, bs.best_ask, bs.mid,
                   bs.sampled_at, bs.local_hour
            FROM book_snapshots bs
            INNER JOIN (
                SELECT condition_id, MAX(sampled_at) AS max_ts
                FROM book_snapshots
                GROUP BY condition_id
            ) latest ON latest.condition_id = bs.condition_id
                     AND latest.max_ts = bs.sampled_at
        """).fetchall()
        latest_by_cid = {
            r[0]: {"best_bid": r[1], "best_ask": r[2], "mid": r[3],
                   "sampled_at": str(r[4]), "local_hour": r[5]}
            for r in latest_raw
        }
        for b in buckets:
            b.update(latest_by_cid.get(b["condition_id"]) or {})

        # Best-guess bucket: highest mid
        mids = [(b["condition_id"], b.get("mid") or 0) for b in buckets]
        best_cid = max(mids, key=lambda x: x[1])[0] if mids else None

        # Latest observed temp
        ...
        return {
            ...
            "latest_obs_c": latest_obs_c,
            "latest_forecast_c": latest_forecast_c,
        }
    except Exception as exc:
