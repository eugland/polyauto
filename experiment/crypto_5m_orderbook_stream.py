#!/usr/bin/env python3
"""
Real-time order book streamer for Polymarket crypto 5m markets.
Streams top of book (bid/ask) and spread width for BTC or ETH.

Usage:
    python -m experiment.crypto_5m_orderbook_stream --asset BTC
    python -m experiment.crypto_5m_orderbook_stream --asset ETH --poll 1
"""
...
log = logging.getLogger("crypto5m.orderbook")
...
def _fetch_books(token_ids: list[str]) -> dict[str, dict]:
    """Fetch order books for a list of token IDs."""
    if not token_ids:
        return {}
    try:
        r = requests.post(
        ...
        log.warning("Book fetch error: %s", exc)
        return {}
...
def _get_current_5m_markets(asset: str) -> tuple[dict[str, str], dict[str, str]] | None:
    ...
    Returns (market_data, token_ids) or None if not found.
    """
    now_ts = int(datetime.now(timezone.utc).timestamp())
    bucket = (now_ts // 300) * 300

    # Try the three possible buckets (current, previous, next)
    for delta in (0, -1, 1):
...
def stream(asset: str, poll_interval: float = 1.0) -> None:
    """Stream order books for the given asset in real-time."""
    log.info("Starting %s 5m order book stream (poll every %.1fs)...", asset.upper(), poll_interval)
        ...
        while True:
            ...
            if now - last_refresh >= 60:
                ...
                if result:
                    ...
                    log.warning("No active %s 5m market found", asset.upper())
                    markets = token_ids = None
                last_refresh = now

            # Stream if we have markets
            if markets and token_ids:
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Real-time order book streamer for Polymarket crypto 5m markets"
    )
    p.add_argument(
        "--asset",
        type=str.upper,
    ...
    stream(asset=args.asset, poll_interval=args.poll)
